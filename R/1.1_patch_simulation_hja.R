...
# RHESSys Inputs
input_rhessys <- list()
input_rhessys$rhessys_version <- "bin/rhessys5.20.1_self_thinning"
input_rhessys$tec_file <- "ws_hja/tecfiles/hja_patch_simulation.tec"
input_rhessys$world_file <- "ws_hja/worldfiles/hja_2can_patch_6018_ts.world"
input_rhessys$world_hdr_prefix <- "hja_2can_patch_6018"
input_rhessys$flow_file <- "ws_hja/flowtables/hja_patch_6018.flow"
input_rhessys$start_date <- "1959 10 1 1"
input_rhessys$end_date <- "2159 10 1 1"
...
input_standard_par_list <- list(
  m = c(1.792761,4, n_sim),
  k = c(1.566492, 4, n_sim),
  m_v = c(1.792761, 4, n_sim),
  k_v = c(1.566492, 4, n_sim),
  pa = c(1, 10, n_sim),
  po = c(0.3, 1.2, n_sim),
...
input_dated_seq_list[[1]] <- data.frame(name="cs2met_dated",type="pspread",year=1959,month=10,day=7,hour=1,value=0.95,stringsAsFactors=FALSE)
#input_dated_seq_list[[1]][2,] <- data.frame(name="cs2met_dated",type="biomass_removal_percent",year=2059,month=10,day=2,hour=1,value=0.25,stringsAsFactors=FALSE)
#input_dated_seq_list[[1]] <- data.frame(name="cs2met_dated",type="biomass_removal_percent",year=2058,month=10,day=2,hour=1,value=0.85,stringsAsFactors=FALSE)


# Make tec-file
#input_tec_data <- NULL
input_tec_data <- data.frame(year=integer(),month=integer(),day=integer(),hour=integer(),name=character(),stringsAsFactors=FALSE)
input_tec_data[1,] <- data.frame(1959, 10, 1, 1, "print_daily_on", stringsAsFactors=FALSE)
input_tec_data[2,] <- data.frame(1959, 10, 1, 2, "print_daily_growth_on", stringsAsFactors=FALSE)
input_tec_data[3,] <- data.frame(2059, 10, 1, 1, "redefine_world", stringsAsFactors=FALSE)
input_tec_data[4,] <- data.frame(2100, 10, 1, 1, "output_current_state", stringsAsFactors=FALSE)
...
output_variables[1,] <- data.frame("height", "awks/output_var_cd_height.awk","patch_sim_stratum.daily",stringsAsFactors=FALSE)
