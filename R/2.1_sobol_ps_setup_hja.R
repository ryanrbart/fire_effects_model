...
input_sobol <- lapply(seq(1,2), function(x){
  ...
  tmp1 <- setNames(data.frame(runif(n_sim, min=6,max=8)), "ws_hja/defs/patch_hja.def:overstory_height_thresh")
  tmp2 <- setNames(data.frame(runif(n_sim, min=4,max=5)), "ws_hja/defs/patch_hja.def:understory_height_thresh")
  ...
  tmp5 <- setNames(data.frame(runif(n_sim, min=-20,max=-1)), "ws_hja/defs/veg_hja_understory.def:overstory_mort_k1")
  tmp6 <- setNames(data.frame(runif(n_sim, min=0.2,max=2)), "ws_hja/defs/veg_hja_understory.def:overstory_mort_k2")
  ...
  tmp10 <- setNames(data.frame(runif(n_sim, min=0.2,max=2)), "ws_hja/defs/veg_hja_7dougfir.def:overstory_mort_k2")
...
write.csv(sobol_model_martinez$X, RHESSYS_PAR_SOBOL_2.1_HJA, row.names = FALSE, quote=FALSE) # Parameter sets
saveRDS(sobol_model_martinez, RHESSYS_PAR_SOBOL_MODEL_MARTINEZ_2.1_HJA) # Empty model
saveRDS(sobol_model_2002, RHESSYS_PAR_SOBOL_MODEL_2002_2.1_HJA) # Empty model
saveRDS(sobol_model_2007, RHESSYS_PAR_SOBOL_MODEL_2007_2.1_HJA) # Empty model
saveRDS(sobol_model_jansen, RHESSYS_PAR_SOBOL_MODEL_JANSEN_2.1_HJA) # Empty model
