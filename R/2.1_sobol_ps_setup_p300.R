...
input_sobol <- lapply(seq(1,2), function(x){
  ...
  tmp1 <- setNames(data.frame(runif(n_sim, min=6,max=8)), "ws_p300/defs/patch_p300.def:overstory_height_thresh")
  tmp2 <- setNames(data.frame(runif(n_sim, min=4,max=5)), "ws_p300/defs/patch_p300.def:understory_height_thresh")
...
write.csv(sobol_model_martinez$X, RHESSYS_PAR_SOBOL_2.1_P300, row.names = FALSE, quote=FALSE) # Parameter sets
saveRDS(sobol_model_martinez, RHESSYS_PAR_SOBOL_MODEL_MARTINEZ_2.1_P300) # Empty model
saveRDS(sobol_model_2002, RHESSYS_PAR_SOBOL_MODEL_2002_2.1_P300) # Empty model
