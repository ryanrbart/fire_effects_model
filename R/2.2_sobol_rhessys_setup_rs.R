# Code for running RHESSys using Sobol Sensitivity Parameters (RS)
#
...
sobol_runs_rs <- function(core_par_path,
  ...
  # input_dated_seq_list[[1]] <- data.frame(name="misreal",type="pspread",year=1988,month=10,day=7,hour=1,value=sobol_model$X$pspread[aa],stringsAsFactors=FALSE)
  ...
  system.time(
    for (aa in seq_len(nrow(sobol_ps))){
      ...
      input_def_list[[10]] <- list(input_hdr_list$soil_def[1], "overstory_height_thresh", sobol_ps$`ws_rs/defs/patch_rs.def:overstory_height_thresh`[aa])
      input_def_list[[11]] <- list(input_hdr_list$soil_def[1], "understory_height_thresh", sobol_ps$`ws_rs/defs/patch_rs.def:understory_height_thresh`[aa])
      # Lower canopy fire parameters
      input_def_list[[12]] <- list(input_hdr_list$stratum_def[1], "understory_mort", sobol_ps$`ws_rs/defs/veg_rs_shrub.def:understory_mort`[aa])
      input_def_list[[13]] <- list(input_hdr_list$stratum_def[1], "consumption", sobol_ps$`ws_rs/defs/veg_rs_shrub.def:consumption`[aa])
      input_def_list[[14]] <- list(input_hdr_list$stratum_def[1], "overstory_mort_k1", sobol_ps$`ws_rs/defs/veg_rs_shrub.def:overstory_mort_k1`[aa])
      input_def_list[[15]] <- list(input_hdr_list$stratum_def[1], "overstory_mort_k2", sobol_ps$`ws_rs/defs/veg_rs_shrub.def:overstory_mort_k2`[aa])
